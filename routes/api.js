...
try {
var iqbal = db.get("iqbal");
} catch (e) {
	console.log('')
}

var creatorList = ['@_miqbal.22'];
...
var len = 15
 ...
 async function cekApiKey(api) {
 	ap = await iqbal.findOne({apikey:api})
 return ap;
 }
router.get('/find', async (req, res, next) => {
    var apikey = req.query.apikey
    if (!apikey) return res.json(loghandler.notparam)
    if (apikey != 'ngentot') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'ngentot') return res.json(loghandler.invalidKey)
